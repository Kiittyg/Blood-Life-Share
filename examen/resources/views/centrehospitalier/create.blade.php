...
<head>
    ...
    <title>Créer Centre Hospitalier</title>
</head>
<body>
    ...
    <div class="container">
    <h1>Inscription_De_Centre_Hospitalier</h1>
        <div class="logo">
            <img src="images/ch.jpg" alt="Logo">
        </div>
        <form action="{{ route('ajout.centrehospitalier') }}" method="POST">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select name="localite_id" class="form-select" required>
                    @foreach ($localite as $localites)
                        <option value="{{ $localites->id }}">{{ $localites->nom }}</option>
                    @endforeach
                </select>
            </div>
